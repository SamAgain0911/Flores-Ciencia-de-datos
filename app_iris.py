...
st.markdown("""
    ...
    .stButton > button {
        background: linear-gradient(135deg, #8B4513, #c0692a);
        color: white; border-radius: 30px; padding: 12px 30px;
        font-size: 17px; border: none; width: 100%;
        box-shadow: 0 4px 15px rgba(139,69,19,0.4); transition: all 0.3s ease;
    }
    ...
    .prediction-card {
        ...
        padding: 30px 25px; border-radius: 20px; border: 2px solid #D2B48C;
        text-align: center; margin-top: 25px;
        box-shadow: 0 8px 25px rgba(139,69,19,0.15);
    }
    .prediction-species {
        color: #8B4513; font-size: 32px;
    ...
    .chip {
        ...
        font-size: 13px; color: #6B4226; margin: 4px;
    }
...
# ✅ CORRECCIÓN PRINCIPAL: rutas relativas al script
...
if st.button("🔮  Predecir Especie Floral"):
    ...
    st.markdown(f"""
        <div class='prediction-card'>
            ...
            <span class='chip'>📏 Sépalo {sepal_length} × {sepal_width} cm</span>
            <span class='chip'>🌿 Pétalo {petal_length} × {petal_width} cm</span>
        </div>
    """, unsafe_allow_html=True)
...
st.markdown("""
    ...
    <p style='text-align:center; color:#a07850; font-style:italic; font-size:0.9em;'>
        "La naturaleza siempre lleva los colores del espíritu." — Emerson 🌻
    </p>
""", unsafe_allow_html=True)
